...
} from "../services/userPreferencesApi.ts";
import type {
...
export const useGetUserPreferences = (id: UserID | undefined) => {
  return useQuery<
    ...
    AxiosError,
    GetUserPreferencesResponse,
    readonly ["user_preferences", UserID]
  >({
...
export const useUpdateUserPreferences = (
  options: UpdateUserPreferencesOptions
) => {
  return useMutation<
    UpdateUserPreferencesResponse,
    AxiosError,
  ...
  >({
    ...
    onSuccess: (data, variables) => {
      toastSuccess("Your Preferences Saved Sucessfully");
      void queryClient.invalidateQueries({
        queryKey: userPreferencesQueryKeys.preferences(variables.id!),
      });
      options?.onSuccess?.(data);
    },
    onError: (error) => {
      let errorData: ErrorResponse | null = null;
      toastError("Error Updating the Preferences");
