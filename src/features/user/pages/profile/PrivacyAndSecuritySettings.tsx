...
const PrivacyAndSecuritySettings = () => {
  const [resetPasswordPayload, setResetPasswordPayload] =
    useState<UpdateUserPasswordPayload>({
      currentPassword: "",
      newPassword: "",
    });
  ...
  const updatePasswordMutation = useUpdatePassword({
    onSuccess: () => {
      ...
      toastSuccess("Password Updated Sucessfully");
      navigate(-1);
    },
  ...
  const handleResetPassword = (e: React.FormEvent) => {
    ...
    if (
      userId &&
      validatePassword(resetPasswordPayload.newPassword) &&
      validatePassword(resetPasswordPayload.currentPassword)
    ) {
      ...
      <div className="border-1 border-[#E8F2E8] w-full p-4 rounded-lg">
        ...
        <form onSubmit={handleResetPassword}>
          ...
          <Button
            ...
            variant={"primary"}
            customClasses={
              "text-base mt-3 font-semibold flex items-center w-[200px]"
            }
